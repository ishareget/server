...
class Notification {
    ...
    async findById(ctx) {
        try {
            const connection = await mysql.createConnection(config);
            const sql = await connection.query('UPDATE web_notification SET status = 1 Where id = ?', [ctx.request.body.id]);
    ...
    async update(ctx) {
        try {
            const connection = await mysql.createConnection(config);
            const [result] = await connection.query(`UPDATE web_notification SET status = 1 Where username = '${ctx.request.body.username}'`);
            return result;
        } catch (e) {
            return false;
        }
