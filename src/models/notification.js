...
class Notification {
    ...
    async create(ctx) {
        try {
            ...
            const sql = `INSERT INTO web_notification (username, groupid, mission_id, noti_time, discription, status, type) VALUES ('${data.username}', ${data.groupid}, ${data.mission_id}, '${data.noti_time}', '${data.discription}', 0, '${data.type}')`;
            const connection = await mysql.createConnection(config);
